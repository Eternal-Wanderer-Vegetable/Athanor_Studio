...
界面结构：顶栏（品牌/文档名/命令搜索/保存）→ 应用菜单（文件/编辑/视图/插入/
格式/工具/帮助）→ 页签功能区（开始/插入/布局/引用/视图/审阅 + 上下文“表格”）→
查找栏 → 左导航轨 + 导航面板（大纲/页面/批注）+ 文档工作区 + 右侧检查面板
（属性/修订历史/语义标注/损失盘点/容器警告）→ 状态栏（字数/保存态/页码/视图
模式/缩放）。

- **单一命令契约**：菜单、功能区、命令面板（Ctrl+K）、快捷键、浮动格式条、
  状态栏全部投影 `CommandRegistry` 的同一份 `Command` 元数据；任何入口不单独
  实现命令逻辑。命令目录与启用条件见 `docs/editor-word-compatibility.md`。
- **能力投影**：能力不满足的命令经 `requiredCapabilities`/`visible` 不渲染入口
  （HTTP 模式没有文件对话框/后台任务）；条件不满足的命令禁用并给出
  `disabledReason`。
- **上下文页签**：光标/选区进入表格自动选中“表格”页签（边沿触发，表格内
  手动切走不被拉回）；离开表格恢复之前页签。浮动格式条只在非空文本选区出现。
- **导航面板**：大纲/页面/批注三个区共用左面板；页面区只显示最近一次成功
  预览的真实 LayoutIndex（页码→块跳转），无分页数据时显示可解释空态；批注区
  同样只显示真实能力状态，不放假线程。
- **视图模式**：状态栏与视图页签的 连续/页宽/页面 三命令；页宽是 CSS zoom 计算
  的适配缩放（切回连续恢复原缩放），页面视图由打印预览浮层承担。
- **剪贴板**：原生 Ctrl+X/C/V 与 Ctrl+Shift/Alt+V 走 handleDOMEvents +
  pendingPasteMode；菜单入口经 Clipboard API 走同一 sanitize/资产路径。
- **响应式**：功能区按容器宽度把低优先级组收进“更多”（无横向滚动），
  溢出菜单里按钮和 picker（字体/字号/颜色/高亮/行距/样式）都可达；
